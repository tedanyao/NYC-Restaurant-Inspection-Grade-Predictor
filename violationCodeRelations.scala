...
//         Array(0,0,0)
// }
def maxProbGrade(a: Array[Int]): String = {
    if (a(0) >= a(1) && a(0) >= a(2)) {
        "A"
...
// examine each violation
val vioGroupRDD = vioRDD.map(x => (x._1, vectorizeGrade(x._3))).
    reduceByKey((a, b) => Array(a(0)+b(0), a(1)+b(1), a(2)+b(2))).
    mapValues(arr => ((arr(0).toDouble / arr.sum, arr(1).toDouble / arr.sum, arr(2).toDouble / arr.sum), maxProbGrade(arr)))
