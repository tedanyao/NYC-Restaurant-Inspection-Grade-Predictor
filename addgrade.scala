...
def transfer(arr: Seq[Any]): Seq[Any] = {
    val key = arr(11).toString + arr(21).toString
    arr.updated(25, myMap.getOrElse(key, ""))
}

val m1 = df.rdd.map(_.toSeq).map(transfer(_))
m1.persist
// m1.map(row => (row(11), row(21), row(25))).take(250).foreach(println)
val correctGradeRDD = m1.filter(row => row(25).toString != "").filter(
    row => !row(28).toString.contains("reopen") && !row(28).toString.contains("Non-operational")) // use this
